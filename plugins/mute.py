...
async def startmute(event):
    xx = await event.eor("**Gua Mute Lu Anjing...**")
    ...
    await xx.eor("**Mampus Dah Gua Mute Lu Kontol...**", time=3)
...
async def endmute(event):
    xx = await event.eor("**Bentar Gua Buka Dulu Mutenya...**")
    if input_ := event.pattern_match.group(1).strip():
        try:
            userid = await event.client.parse_id(input_)
        except Exception as x:
    ...
    await xx.eor("**Dah Gua Lepas Mutenya Nyett Bilang Apa Lu Anjing...**", time=3)
...
async def _(e):
    xx = await e.eor("**Gua Mute Lu Anjing...**")
    huh = e.text.split()
    try:
        tme = huh[1]
    except IndexError:
    ...
    try:
        ...
        await eod(
            xx,
            f"**Mampus Dah Gua Mute Lu Anjing...** [{name}](tg://user?id={userid}) `in {chat.title} for {tme}`",
            time=5,
        )
...
async def _(e):
    xx = await e.eor("**Bentar Gua Lepas Dulu Mutenya...**")
    ...
    try:
        ...
        await eod(
            xx,
            f"**Dah Gua Lepas Mutenya Nyett Bilang Apa Anjing...** [{name}](tg://user?id={userid}) `in {chat.title}`",
            time=5,
        )
    except BaseException as m:
        await xx.eor(f"`{m}`", time=5)
...
async def _(e):
    xx = await e.eor("**Mampus Gua Mute Lu Anjing...**")
    input = e.pattern_match.group(1).strip()
    chat = await e.get_chat()
    if e.reply_to_msg_id:
    ...
    try:
        ...
        await eod(
            xx,
            f"**Mampus Dah Gua Mute Lu Anjing...** {name} `in {chat.title}`",
        )
    except BaseException as m:
        await xx.eor(f"`{m}`", time=5)
